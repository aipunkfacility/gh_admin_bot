...
### 🔍 ПРАВИЛО 7: СОХРАННОСТЬ SEO-ВЕРИФИКАЦИИ

- **ЗАПРЕЩЕНО** удалять файл верификации (например, `public/google*.html`) и мета-теги подтверждения в `SEO.astro`.
- Эти элементы необходимы для поддержания статуса владельца в Google Search Console.
- При рефакторинге папки `public/` файл должен быть сохранен.
